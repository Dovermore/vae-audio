...
# make mu-law transform
class Quantization:
    def __init__(self, min, max, quantization_channels=256):
        self.min = min
        self.max = max
        self.quantization_channels = quantization_channels
        self.bins = np.linspace(self.min, self.max, self.quantization_channels)

    def __call__(self, x):
        inds = np.digitize(x, self.bins, right=True)
        return inds
